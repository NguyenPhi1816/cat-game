...
export default function CatsScreen({ navigation }: CatListScreenProps) {
  ...
  const onRefresh = useCallback(() => { setRefreshing(true); fetchCats(); }, [fetchCats]);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>No cats yet. Add your first one!</Text>}
        ListFooterComponent={
          cats.length < 3 ? (
            <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('CreateCat')}>
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            style={styles.card}
            onPress={() => navigation.navigate('CatDetail', { catId: item.id, catName: item.name })}
          >
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', margin: 8, marginHorizontal: 16, padding: 14, borderRadius: 12, elevation: 2 },
  ...
  addBtn: { margin: 16, backgroundColor: '#6c63ff', borderRadius: 10, padding: 14, alignItems: 'center' },
