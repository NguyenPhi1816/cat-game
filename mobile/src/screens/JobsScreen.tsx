    ...
    <View style={styles.activeCard}>
      <View>
        ...
        <Text style={styles.jobMeta}>
          💰 {activeJob.job?.reward_money} coins
        </Text>
      ...
      {done && (
        <TouchableOpacity
          ...
          onPress={() => onCollect(activeJob.id)}
        >
...
export default function JobsScreen({ navigation }: JobsScreenProps) {
  ...
  const handleAssign = useCallback(
    async (catId: string) => {
      ...
      setAssigning(true);
      try {
        await apiClient.post("/quests/assign", {
          cat_id: catId,
        ...
        Alert.alert(
          "Failed",
          "Cat may already be on a job or have low energy.",
        );
      } finally {
        setAssigning(false);
      }
        ...
        {jobs.map((job) => (
          <View key={job.id} style={styles.jobCard}>
            <View>
              ...
              <Text style={styles.jobMeta}>
                💰 {job.reward_money} coins · ⏱ {job.duration}min
              </Text>
            </View>
            <TouchableOpacity
              style={styles.sendBtn}
              onPress={() => setAssigningJob(job)}
            >
          ...
          <View style={styles.modalCard}>
            ...
            {cats.map((cat) => (
              <TouchableOpacity
                ...
                style={[
                  styles.catOption,
                  cat.energy < 0.3 && styles.catOptionDisabled,
                ]}
                onPress={() => handleAssign(cat.id)}
                disabled={cat.energy < 0.3 || assigning}
              >
                <Text style={styles.catOptionName}>🐱 {cat.name}</Text>
                <Text style={styles.catOptionEnergy}>
                  ⚡ {(cat.energy * 100).toFixed(0)}%
                  {cat.energy < 0.3 ? " (too tired)" : ""}
                </Text>
              </TouchableOpacity>
            ))}
            <TouchableOpacity
              style={styles.cancelBtn}
              onPress={() => setAssigningJob(null)}
            >
