...
import { Modal, RefreshControl } from "../components/NativeWrappers";
...
export default function HomeScreen() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const hasReport = reportRes.data?.has_report;
      setShowReport(hasReport === true || hasReport === "true");
    } catch {
      ...
      >
        ...
        {cats.length === 0 && (
          <Text style={styles.empty}>
            No cats yet. Go to the Cats tab to add one!
          </Text>
        )}
        {cats.map((cat) => (
          <View key={cat.id} style={styles.catCard}>
            ...
            <Text style={styles.catMeta}>
              Lv.{cat.level} · {cat.personality_type}
            </Text>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalText}>
              You were away for {report?.hours_away?.toFixed(1)} hours
              {report?.capped ? " (capped at 8h)" : ""}
            </Text>
            <Text style={styles.modalText}>
              💰 Salary earned: {report?.salary_earned} coins
            </Text>
            {(Array.isArray(report?.cat_reports) ? report.cat_reports : []).map(
              (cr, i) => (
                <View key={i} style={styles.catReport}>
                  <Text style={styles.catReportName}>
                    {cr.cat_name} +{cr.xp_gained} XP
                    {cr.leveled_up ? " 🎉 Level Up!" : ""}
                  </Text>
                  {(Array.isArray(cr.actions_performed)
                    ? cr.actions_performed
                    : []
                  ).map((a, j) => (
                    <Text key={j} style={styles.catAction}>
                      • {a}
                    </Text>
                  ))}
                </View>
